...
- 页面关闭开始计时、超时清理，以及超时前重新打开取消计时时，后端都会输出对应日志，记录用户、开始触发时间、超时时长和超时时间
...
| 认证 | POST | `/api/auth/window-close-start` | 是 | 记录同浏览器最后一个系统页面关闭后的超时开始日志 |
...
- 当最后一个页面关闭开始计时，以及后续超时清理发生时，后端操作日志都会记录对应时间信息，便于排查
